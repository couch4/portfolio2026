...
export const WaterMaterial = shaderMaterial(
  {
    ...
    iSeaSpeed: 0.6,
    iSeaFreq: 0.24,
    iSeaBase: new Vector3(0.11,0.19,0.22),
    iFlySpeed: 1.0,
    iNight: false,
  },
  // vertex shader
  ...
  /*glsl*/ `    
    ...
    uniform float iSeaSpeed;
    uniform float iSeaFreq;
    uniform vec3 iSeaBase;
    uniform float iFlySpeed;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    SEA_HEIGHT = iSeaHeight;
    SEA_CHOPPY = iSeaChoppy;
    SEA_SPEED = iSeaSpeed;
    SEA_FREQ = iSeaFreq;
    SEA_BASE = iSeaBase;
   ...
   // vec3 ang = vec3(sin(time*3.0)*0.1,sin(time)*0.2+0.3,time);
    //float roll = PI + sin(iTime)/30.0 + cos(iTime/2.0)/30.0 ;
    float roll = PI;
    float pitch = PI * iPitch + (sin(iTime/2.0)+ cos(iTime))/40.0 
        + (iResolution.y - .8)*PI/3.0  ;
    float yaw = iResolution.x * PI * 4.0;
   ...
   // vec3 ang = vec3(roll,pitch,0);
    ...
    vec3 ori = vec3(0.0,iHeight,time*0.5*iFlySpeed);

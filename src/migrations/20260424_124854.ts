...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`experience_achievements\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`achievement\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`experience_achievements_order_idx\` ON \`experience_achievements\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`experience_achievements_parent_id_idx\` ON \`experience_achievements\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`skills_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`skills_rels_order_idx\` ON \`skills_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`skills_rels_parent_idx\` ON \`skills_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`skills_rels_path_idx\` ON \`skills_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`skills_rels_projects_id_idx\` ON \`skills_rels\` (\`projects_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`experience_achievements\`;`)
  ...
  await db.run(sql`DROP TABLE \`skills_rels\`;`)
